{lib, ...}: {
  ...
  in
    ...
    // badChecks
    // {
      roundtrip = pkgs: (import ./testsuite.nix {inherit pkgs;}).roundtrip;
      list = pkgs: (import ./testsuite.nix {inherit pkgs;}).list;
      filters = pkgs: (import ./testsuite.nix {inherit pkgs;}).filters;
      fuzz = pkgs: (import ./testsuite.nix {inherit pkgs;}).fuzz;
    };
}
