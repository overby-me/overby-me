...
|-----------|----------|-----------------|--------|-------|--------|
| Shell | bash | [oxidized-bash](../bash) | repo | 1 | ✅ Available |
| Core utilities | coreutils | [uutils](https://github.com/uutils/coreutils) | nixpkgs | 1 | ✅ Available |
| Stream editor | gnused | [oxidized-sed](../sed) | repo | 2 | ✅ Available |
| Pattern grep | gnugrep | safety/oxidized/grep | repo | 2 | ✅ Available |
| Awk | gawk | safety/oxidized/awk | repo | 2 | ✅ Available |
| File search | findutils | [uutils-findutils](https://github.com/uutils/findutils) | nixpkgs | 2 | ✅ Available |
| Diff | diffutils | [oxidized-diffutils](../diffutils) | repo | 2 | ✅ Available |
| Tar archive | gnutar | safety/oxidized/tar | repo | 3 | ✅ Available |
| Gzip | gzip | safety/oxidized/gzip | repo | 3 | ✅ Available |
| Bzip2 | bzip2 | safety/oxidized/bzip2 | repo | 3 | ✅ Available |
| XZ/LZMA | xz | safety/oxidized/xz | repo | 3 | ✅ Available |
| Build driver | gnumake | safety/oxidized/make | repo | 4 | ✅ Available |
| Patch | gnupatch | safety/oxidized/patch | repo | 4 | ✅ Available |
| ELF patcher | patchelf | safety/oxidized/patchelf | repo | 5 | ✅ Available |
...
1. **Component files, not crate stubs.** Each `components/*.nix` file declares the replacement mapping. Actual Rust rewrites live under `rust/` (e.g. `safety/oxidized/make/`) or come from nixpkgs (e.g. `pkgs.uutils-coreutils-noprefix`).
...
|------|-------------|-------|
| bash | oxidized-bash (`pkgs.oxidized-bash`) | Bash-compatible shell written in Rust; provides `/bin/bash` and `/bin/sh` |
...
| GNU make | 4.4.1 | autotools | Built with oxidized-make (self-referential!) |
| GNU awk (gawk) | 5.3.1 | autotools + yacc | Parser generated by oxidized-bison! |
| GNU bc | 1.07.1 | autotools + yacc + flex | Calculator, exercises yacc/flex/ed |
| GNU findutils | 4.10.0 | autotools | find/xargs/locate/updatedb |
| GNU tar | 1.35 | autotools | Built with oxidized-tar (self-referential!) |
| GNU gzip | 1.14 | autotools | Built with oxidized-gzip (self-referential!) |
| XZ Utils | 5.6.4 | autotools | Built with oxidized-xz (self-referential!) |
...
- [x] Declare shell component with oxidized-bash replacement
...
- [x] Validate oxidized-bash can execute stdenv's `setup.sh` phases without modification — **setup.sh loads successfully**, all 63 functions defined including `genericBuild` and all build phase functions

### Known Incompatibilities

1. **oxidized-bash as stdenv shell is experimental** — oxidized-bash can now source and execute `setup.sh` (all functions load correctly), but running full builds as the shell requires further testing. Key features implemented: `[[ ]]`, arrays, `(( ))`, nameref, indirect expansion, process substitution, `exec {fd}<`, FUNCNAME, local variable scoping.
...
- **oxidized-bash compatibility:** oxidized-bash is still maturing; some bash-isms in `setup.sh` or configure scripts may fail. Build-time usage may hit edge cases not covered by interactive use.
...
|------|-------------|--------|-------|
| gnused | oxidized-sed | repo | Full GNU sed replacement — supports all delimiters (including &), BRE/ERE, in-place editing, hold space, N command, branch/label. Replaces uutils-sed which had critical `&` delimiter bug. |
| gnugrep | safety/oxidized/grep | repo | GNU-flag-compatible with BRE/ERE/PCRE, -w, -c, -l, -L, context |
| gawk | safety/oxidized/awk | repo | Lexer/parser/interpreter with POSIX awk + GNU extensions |
| findutils | uutils-findutils | nixpkgs | From [uutils/findutils](https://github.com/uutils/findutils), runs GNU testsuite |
| diffutils | oxidized-diffutils | repo | Myers diff algorithm with normal/unified/context/ed/rcs output, diff/cmp/sdiff/diff3 via argv[0] detection |
...
|------|-------------|-----------------|
| gnutar | safety/oxidized/tar | `tar` crate — GNU-compatible CLI wrapper |
| gzip | safety/oxidized/gzip | `flate2` crate — gzip/gunzip/zcat |
| bzip2 | safety/oxidized/bzip2 | `bzip2` crate — bzip2/bunzip2/bzcat |
| xz | safety/oxidized/xz | `xz2` crate — xz/unxz/xzcat/lzma/unlzma/lzcat |
...
|------|-------------|--------|
| gnumake | safety/oxidized/make | ✅ Available — suffix rules, pattern rules, recursive variables, nested expansion, continuation lines. Successfully builds GNU coreutils (106 programs). |
| gnupatch | safety/oxidized/patch | ✅ Available — unified/context/normal diff with fuzz matching |

### Testing Strategy

- Patch application testing on the full set of nixpkgs patches
- Differential testing on Makefile corpora from nixpkgs packages (when oxidized-make is attempted)
...
| Tool | Replacement | Rust Foundation |
|------|-------------|-----------------|
| patchelf | safety/oxidized/patchelf | `goblin` crate for ELF parsing/writing |
...
stdenvRs = pkgs.stdenv.override {
  initialPath = [
    oxidized-bash              # Phase 1
    uutils-coreutils       # Phase 1
    uutils-sed             # Phase 2 (from uutils project)
    oxidized-grep              # Phase 2
    oxidized-awk               # Phase 2
    rust-findutils         # Phase 2
    oxidized-diffutils         # Phase 2
    oxidized-tar               # Phase 3
    oxidized-gzip              # Phase 3
    oxidized-bzip2             # Phase 3
    oxidized-xz                # Phase 3
    oxidized-make              # Phase 4
    oxidized-patch             # Phase 4
  ];
...
All Rust rewrites live under `rust/` in the monorepo (e.g. `safety/oxidized/sed`, `safety/oxidized/grep`).
Do **not** use `-rs` suffixes in directory names — the `rust/` prefix is sufficient.
Do **not** touch bash or make replacements yet.

When creating a new Rust rewrite (e.g. `safety/oxidized/sed`):
...
2. **Wire it into rust-nixpkgs** by updating the component file:
   ...
   ```nix
   # safety/oxidized/nixpkgs/components/sed.nix
   mkComponent {
     name = "gnused";
...
3. **Import the subproject** in the root `flake.nix`:
   ...
   Commitlint scopes are auto-derived from directory structure, so `safety/oxidized/sed`
   is automatically available as a scope.

4. **Test** with `just test` in `safety/oxidized/nixpkgs/`.
...
| [oxidized-pkg-config](../pkg-config) | Rust pkg-config replacement — already used in the build toolchain |
