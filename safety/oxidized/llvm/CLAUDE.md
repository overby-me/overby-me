...
| **Library crates stay dependency-free and stable-Rust for as long as that is free.** Adding a crate means a lock commit and the tarballs behind it; adding nightly means a toolchain pin. Both are fine when a tier needs them, neither is free. | Today the whole workspace builds with zero third-party crates. |
