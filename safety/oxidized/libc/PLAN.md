...
2. Plain source import (no submodules; jj has no story for them anyway) into
   `safety/oxidized/libc/{eyra,c-ward,origin}`. Strip upstream CI/workflows; keep
...
| **P3** | NixOS-rs integration | Substrate consumable from `safety/oxidized/nixos`; static-service scope first; `eyra-c` (upstream's libc.a build) evaluated for residual C packages |
...
- `platform/nix/lib/lib/cargo` (the in-house per-crate/gradual build lib) for the
