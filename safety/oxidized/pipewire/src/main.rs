// oxidized-pipewire: a multicall binary providing every PipeWire tool/daemon.
//
// Dispatched by argv[0] (or argv[1] when invoked via `oxidized-pipewire <tool>`).
...
fn dispatch(name: &str, args: &[String]) -> i32 {
    match name {
        ...
        "spa-inspect" => spa_tools::spa_inspect::main(args),
        "spa-monitor" => spa_tools::spa_monitor::main(args),
        "spa-acp-tool" => spa_tools::spa_acp_tool::main(args),
        ...
        "oxidized-pipewire" => {
            eprintln!(
                "oxidized-pipewire: multicall binary, invoke via a tool symlink or `oxidized-pipewire <tool> [args]`."
            );
            2
        }
        _ => {
            eprintln!("oxidized-pipewire: unknown tool {name:?}");
            2
        }
    }
}
...
fn stub_main(name: &str, args: &[String]) -> i32 {
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        println!("{name}: not yet implemented in oxidized-pipewire");
        return 0;
    }
    eprintln!("{name}: not yet implemented in oxidized-pipewire");
    1
}
...
fn main() -> ExitCode {
    ...
    // If invoked as `oxidized-pipewire <tool> [args]`, dispatch on argv[1] and
    // shift it out so the called tool sees its own name in argv[0].
    let (name, args): (String, Vec<String>) =
        if argv0_name == "oxidized-pipewire" && argv.len() >= 2 {
