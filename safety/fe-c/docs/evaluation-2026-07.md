...
| **`harden` enters the per-crate derivation key** (`docs/nix-integration.md` §6 Q2, flagged as a correctness bug if guessed wrong) | Corpus checks invoke `cargo build` with `RUSTC=` and env vars in a temp `CARGO_TARGET_DIR`. `platform/nix/lib/cargo`'s per-crate key is untouched. | Mode is not in any derivation hash. Today the checks sidestep this by using separate target dirs per mode; the moment the dial is real, this is the stale-artifact bug the plan warned about. |
...
Either build it (the `platform/nix/lib/cargo` per-crate flags patch, `harden` in the
