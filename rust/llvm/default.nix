...
in {
  checks = {
    ...
    llvm-upstream-assembler = pkgs: upstreamCheck pkgs "Assembler" 483 0;
    llvm-upstream-verifier = pkgs: upstreamCheck pkgs "Verifier" 327 0;
    ...
    # Its three remaining files are three causes, each a task of its own: the
    # order attributes taking an argument print in, which of two attribute
    # groups sharing an id wins, and named types reachable only from a debug
    # record.
    llvm-opt-differential = pkgs: differentialCheck pkgs "Assembler" 220;
    llvm-opt-differential-debuginfo = pkgs: differentialCheck pkgs "DebugInfo" 54;
