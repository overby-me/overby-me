...
in {
  checks = {
    ...
    # Its two remaining files are two causes, each a task of its own: which
    # of two attribute groups sharing an id wins, and named types reachable
    # only from a debug record.
    llvm-opt-differential = pkgs: differentialCheck pkgs "Assembler" 220;
    llvm-opt-differential-debuginfo = pkgs: differentialCheck pkgs "DebugInfo" 55;
