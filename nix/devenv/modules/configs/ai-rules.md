...
- **Root config files are copied from `devenv/modules/configs/` — never edit the root copies directly.** The devenv `enterShell` hook (in `devenv/modules/configs/default.nix`) populates the project root on every shell entry. All files are copied (via `install -m 644`) as real, writable files rather than symlinked into the read-only Nix store, so tools and AI agents are never prompted for permission to follow symlinks into `/nix/store`. The mapping is:

  | Root file | Source | Method |
  |-|-|-|
  | `biome.jsonc` | `devenv/modules/configs/biome-nix.jsonc` | copy |
  | `deno.jsonc` | `devenv/modules/configs/deno.jsonc` | copy |
  | `lychee.toml` | `devenv/modules/configs/lychee.toml` | copy |
  | `rumdl.toml` | `devenv/modules/configs/rumdl.toml` | copy |
  | `typos.toml` | `devenv/modules/configs/typos.toml` | copy |
  | `.secretsignore` | `devenv/modules/configs/secretsignore` | copy |
  | `.commitlintrc.yml` | `devenv/modules/configs/commitlintrc.nix` | generated (Nix derivation) — scope list is auto-derived from top-level directory names; copied into place |
  | `.zed/settings.json` | `devenv/modules/configs/zed/settings.jsonc` | copy |
  | `.rules` | `devenv/modules/configs/ai-rules.md` | copy |
  | `.claude/rules/rules.md` | `devenv/modules/configs/ai-rules.md` | copy |
