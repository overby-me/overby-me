...
# nix/devshell/modules/git-hooks.nix.
#
...
pkgs: let
  ...
  # There is no YAML formatter, and the builtin prettier must not become one:
  # .tangled/workflows/*.yml is generated by `nickel export | yq` on every
  # devshell entry, and prettier rewrites yq's `'**'` to `"**"`. The two
...
in {
  ...
  # over *.md. Markdown is still formatted by the rumdl devshell git-hook on
  # commit.
  "*.md" = noop;
  ...
  # Matches the devshell biome hook's types_or (javascript, jsx, ts, tsx,
  ...
  # so `nix fmt` would restyle CSS that devshell leaves alone.
  "*.js" = biomeFormat;
  "*.jsx" = biomeFormat;
  "*.ts" = biomeFormat;
  "*.tsx" = biomeFormat;
  ...
  # still formatted by the dedicated mojo-format devshell git-hook on commit.
}
