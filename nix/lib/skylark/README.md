...
target and action registration). `nix/lib/buck2` is the first consumer; the
interpreter itself knows nothing about Buck2.

## Usage

```nix
let sk = import ./nix/lib/skylark/api.nix;
in {
...
```console
nix eval -f nix/lib/skylark/tests/lexer.nix
nix eval -f nix/lib/skylark/tests/parser.nix
nix eval -f nix/lib/skylark/tests/eval.nix
```
