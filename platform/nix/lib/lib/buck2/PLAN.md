...
- `platform/nix/lib/lib/skylark`: a pure-Nix Starlark interpreter (lexer, parser,
  ...
  unchanged.
- `platform/nix/lib/lib/buck2`: the Buck2 semantics layered on top. It supplies the Buck2
...
## Skylark interpreter (`platform/nix/lib/lib/skylark`)

Builtins-only (no `pkgs`, no nixpkgs `lib` beyond what we pass in), mirroring
`platform/nix/lib/lib/cargo/lib`: unit tests run with a bare
`nix eval -f platform/nix/lib/lib/skylark/tests/<mod>.nix`.
...
## Buck2 layer (`platform/nix/lib/lib/buck2`)

```text
platform/nix/lib/lib/buck2/
  PLAN.md               this file
  README.md             usage docs (mirrors platform/nix/lib/lib/cargo/README.md)
...
- **Skylark unit tests** (`platform/nix/lib/lib/skylark/tests/*.nix`): assert-based,
  ...
  use.
- **Buck2 unit tests** (`platform/nix/lib/lib/buck2/tests/*.nix`): label parsing, cell
...
- **End-to-end checks** (`platform/nix/lib/lib/buck2/checks.nix`, wired individually into
...
- 2026-07-19: Two libraries, `platform/nix/lib/lib/skylark` (reusable interpreter) and
  `platform/nix/lib/lib/buck2` (semantics + lowering), sibling to `platform/nix/lib/lib/cargo` and
  `platform/nix/lib/lib/deno`, exposed via `perSystemLib`. Rationale: Starlark outlives
