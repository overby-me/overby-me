...
in
  {
    ...
    bins ? null,
    release ? true,
    # A cfg platform key like "aarch64-linux". Normal deps resolve and compile
    # for the target, while build scripts, their dependency closures and
    # proc-macros stay on the host. Needs a toolchain carrying the target's std,
    # and a crossCC to link with.
    crossTarget ? null,
    # Cross C compiler (e.g. pkgsCross.aarch64-multiplatform.stdenv.cc).
    crossCC ? null,
    # rmeta pipelining, experimental: dependents compile against dependency
    # metadata so codegen leaves the critical path, and bins still link real
    # rlibs. Needs a nightly toolchain with -Zalways-encode-mir in rustcFlags,
    # because stable's standalone --emit=metadata produces check-grade rmeta
    # without the optimized MIR dependents need for codegen.
    pipeline ? false,
    ...
    toolchain ? null,
    # Compile and run each root member's unit and tests/*.rs targets, with
    # dev-dependencies, exposed as passthru.tests.<member>. The package's own
    ...
    # The mini-index rebuilt from the crates' published Cargo.toml, read out of
    # the same fixed-output .crate tarballs the build already fetches. No network
    # and no sandbox relaxation: the tarballs are content-verified store paths, so
    # the derivation is pure and cacheable, and lib/index.nix reads its output at
    # eval time - the one IFD, forced only when `index` is null. Keyed by the lock,
    # so projects resolving the same crates share it.
    cratesIoPkgs =
    ...
    # The compiler cfg set is a pure function of the toolchain, so run
    # `rustc --print cfg` once rather than in every build-script sandbox.
    rustcCfgFile =
    ...
    # Host-platform variants for the build-dependency world: build scripts and
    # everything they link run on the compiling machine. A native build aliases
    # the main map, so nothing changes there.
    hostDrvs =
      if crossTarget == null
      then drvs
    ...
    # Metadata-only derivations for pipelining. Members are excluded, being roots
    # that build bins; for the rest the sandbox decides eligibility, falling back
    # to a full build for proc-macros and build-script crates.
    rmetaDrvs =
      mapAttrs (
        id: node:
    ...
    # A parallel host-only, dev-inclusive resolution and drv set, kept apart from
    # the main graph so enabling tests never alters the package's derivations.
    resolvedTests =
      if !runTests
      then null
      else
