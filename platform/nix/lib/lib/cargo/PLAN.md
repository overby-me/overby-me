...
`nix eval -f platform/nix/lib/lib/cargo/tests/foo.nix` and the resolver is trivially portable.
...
- Unit: assert-based eval tests per lib module, run directly via
  `nix eval -f platform/nix/lib/lib/cargo/tests/<mod>.nix` and wired as trivial checks.
...
- 2026-07-18: Library lives at `platform/nix/lib/lib/cargo/`, sibling of `platform/nix/lib/lib/deno`;
