...
target and action registration). `platform/nix/lib/lib/buck2` is the first consumer; the
interpreter itself knows nothing about Buck2.

## Usage

```nix
let sk = import ./platform/nix/lib/lib/skylark/api.nix;
in {
...
```console
nix eval -f platform/nix/lib/lib/skylark/tests/lexer.nix
nix eval -f platform/nix/lib/lib/skylark/tests/parser.nix
nix eval -f platform/nix/lib/lib/skylark/tests/eval.nix
```
