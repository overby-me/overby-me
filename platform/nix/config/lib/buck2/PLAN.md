...
`platform/nix/config/lib/cargo` (per-crate derivations from `Cargo.lock`) and `platform/nix/config/lib/deno`,
...
- `platform/nix/config/lib/skylark`: a pure-Nix Starlark interpreter (lexer, parser,
  ...
  unchanged.
- `platform/nix/config/lib/buck2`: the Buck2 semantics layered on top. It supplies the Buck2
...
## Skylark interpreter (`platform/nix/config/lib/skylark`)

Builtins-only (no `pkgs`, no nixpkgs `lib` beyond what we pass in), mirroring
`platform/nix/config/lib/cargo/lib`: unit tests run with a bare
`nix eval -f platform/nix/config/lib/skylark/tests/<mod>.nix`.

```text
platform/nix/config/lib/skylark/
...
## Buck2 layer (`platform/nix/config/lib/buck2`)

```text
platform/nix/config/lib/buck2/
  PLAN.md               this file
  README.md             usage docs (mirrors platform/nix/config/lib/cargo/README.md)
...
- **Skylark unit tests** (`platform/nix/config/lib/skylark/tests/*.nix`): assert-based,
  ...
  use.
- **Buck2 unit tests** (`platform/nix/config/lib/buck2/tests/*.nix`): label parsing, cell
...
- **End-to-end checks** (`platform/nix/config/lib/buck2/checks.nix`, wired individually into
...
- 2026-07-19: Two libraries, `platform/nix/config/lib/skylark` (reusable interpreter) and
  `platform/nix/config/lib/buck2` (semantics + lowering), sibling to `platform/nix/config/lib/cargo` and
  `platform/nix/config/lib/deno`, exposed via `perSystemLib`. Rationale: Starlark outlives
