# Verbatim copy of nixpkgs' installer/cd-dvd/iso-image.nix with
# NixOS/nixpkgs#396334 ("nixos/iso-image: add devicetree support") applied, and
# disabledModules pointing at the original so importing this replaces it.
#
# Why: the stock module generates its GRUB menu entries from a hard-coded
# template with no hook for a `devicetree` line, so on a machine whose UEFI
# hands Linux no devicetree (every Snapdragon X laptop, armitas included) the
# installer only boots if you edit the entry by hand at the menu.  The patch is
...
# re-apply the four changes marked `# armitas:` below and run `nix fmt`.  The
# copy is alejandra-formatted, so it will not diff cleanly against upstream's
# nixfmt-rfc-style original.
{
  config,
  lib,
  utils,
  pkgs,
  # armitas: added, so the two relative imports below can be rewritten to
  # absolute nixpkgs paths.  Upstream resolves them relative to its own
  # location inside the nixpkgs tree, which this copy is not in.
  modulesPath,
  ...
}: let
  ...
  menuBuilderGrub2 = {
    ...
    params,
    initrd,
    # armitas: from NixOS/nixpkgs#396334.
    devicetree,
  }: ''
  ...
  buildMenuGrub2 = {
    ...
    menuConfig = {
      ...
      # armitas: from NixOS/nixpkgs#396334.
      devicetree =
        if cfg.hardware.deviceTree.enable
        then "/boot/${cfg.hardware.deviceTree.package + "/" + cfg.hardware.deviceTree.name}"
        else null;
      class = "installer";
    };
  in ''
...
in {
  imports = [
    ...
    # armitas: was ../../image/file-options.nix, relative to this module's
    # place in the nixpkgs tree.
    (modulesPath + "/image/file-options.nix")
  ];

  # armitas: replace the stock module rather than colliding with it.
  ...
  config = {
    assertions = [
      ...
      (
        let
          badSpecs =
            lib.filterAttrs (
              _specName: specCfg: specCfg.configuration.isoImage.volumeID != config.isoImage.volumeID
            )
    ...
    isoImage.contents = let
      cfgFiles = cfg:
      # armitas: the deviceTree branch is from NixOS/nixpkgs#396334; the
      # surrounding list only gained parentheses to make room for it.
    ...
    # armitas: was ../../../lib/make-iso9660-image.nix.
    system.build.isoImage = pkgs.callPackage (modulesPath + "/../lib/make-iso9660-image.nix") (
      {
