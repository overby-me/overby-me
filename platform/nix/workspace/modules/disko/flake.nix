# declarative disk layout, so a host can be partitioned before it exists.
#
...
{
  description = "declarative disk layout, so a host can be partitioned before it exists";
  ...
  outputs = inputs: {
    workspaceModule = {lib, ...}: {
      ...
      key = "nix-workspace/modules/disko";
      ...
      inputs.disko = lib.mkDefault inputs.disko;
    };
  };
}
