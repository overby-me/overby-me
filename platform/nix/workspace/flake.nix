...
# module so each repo states only what is different about it, rather than
# carrying its own copy of one hook list.
#
# It is itself published, at tangled.org/overby.me/nix-workspace and
# github.com/overby-me/nix-workspace, and the monorepo is the source of
# truth for it as for everything else here.
{
  description = "The shared build for projects published from the overby.me monorepo";
  ...
  outputs = inputs: let
    mkFlake = import ./mk-flake.nix {inherit (inputs) nixpkgs;};
  ...
  in
    mkFlake ./. {
      ...
      outputs = {
        workspaceModules.default = module;
        ...
        # functor, and a monorepo can import it by path instead of taking
        # this flake as an input.
        workspace = import ./workspace.nix;
      };

      # Callable, so a published repo's whole flake is one call rather than a
      # copy of the same inputs, the same import and the same follows. The
      # inputs closed over here are this flake's own, which is what makes
