...
def flake-text [
    ...
    # to be restated in this flake is a buildCargoProject argument there.
    let rust_block = []

    # Every call passes its inputs: that attribute is where the root comes
    # from, since a flake cannot ask itself where it is. `outputDirs` says the
    # root is the project rather than a tree of them, which is what makes the
    # repo's own default.nix the build.
...
def main [--check, --github: string = "overby-me"]: nothing -> nothing {
    ...
    for p in $projects {
        ...
        let want_flake = if not $is_crate { "" } else { (
            flake-text $p.name $description "git+https://tangled.org/overby.me/nix-workspace"
