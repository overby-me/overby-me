...
[`gen-standalone.nu`](./gen-standalone.nu).
...
nu publish.nu setup-github --only rust-foo   # create the GitHub mirror
nu gen-standalone.nu                         # flake, CI and README banner
nu publish.nu
```
...
```text
:[ :/rust/grep:...:prefix=grep, :/rust/pcre2:...:prefix=pcre2, ... ]
```
...
`rust-grep` now picks up commits touching `safety/oxidized/pcre2`, whose history
`rust-pcre2` also publishes on its own. That duplication is the price of a repo
