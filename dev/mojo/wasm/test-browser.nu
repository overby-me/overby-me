...
#
# Inspired by safety/oxidized/nixos/test-boot.nu: build artifact → boot in real environment
...
# ── Logging (same style as safety/oxidized/nixos/test-boot.nu) ─────────────────────────
...
def wd-new-session [] {
    ...
    if $resp == null { return "" }
    let sid = ($resp | get -i value.sessionId | default ($resp | get -i sessionId | default ""))
    $sid
}
