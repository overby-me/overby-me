...
def main [
    ...
    let root_dir = ($project_dir | path dirname)
    let test_dir = ($root_dir | path join "core" "test")
    let core_src_dir = ($root_dir | path join "core" "src")
    ...
    let results = $to_build | par-each --threads $num_jobs { |src|
        ...
        let args = [-I $wasmtime_mojo -I $core_src_dir -I $web_src_dir -I $examples_dir -I $test_dir -o $bin $src]
