...
//! common H.265/HEVC chroma format).  This module provides an optional
//! conversion path to packed RGB/RGBA so that downstream consumers that
//! expect RGB input can use the decoder directly.
...
pub enum PixelFormat {
    ...
    /// width and half height.  This is the native output of the H.265
...
/// BT.2020 YUV to RGB conversion (UHD / HDR content).
///
/// Uses the BT.2020 non-constant-luminance limited-range matrix:
///
/// ```text
/// R = clip(( 298*(Y-16)                + 430*(Cr-128) + 128) >> 8)
/// G = clip(( 298*(Y-16) -  48*(Cb-128) - 167*(Cr-128) + 128) >> 8)
/// B = clip(( 298*(Y-16) + 548*(Cb-128)                + 128) >> 8)
/// ```
#[inline]
pub fn yuv_to_rgb_bt2020(y: u8, cb: u8, cr: u8) -> (u8, u8, u8) {
    let y = y as i32 - 16;
    let cb = cb as i32 - 128;
    let cr = cr as i32 - 128;

    let r = (298 * y + 430 * cr + 128) >> 8;
    let g = (298 * y - 48 * cb - 167 * cr + 128) >> 8;
    let b = (298 * y + 548 * cb + 128) >> 8;
...
pub enum ColourMatrix {
    ...
    Bt601,
    /// ITU-R BT.709 (HD content).  Default for H.265/HEVC.
    #[default]
    Bt709,
...
impl ColourMatrix {
    ...
    pub fn yuv_to_rgb(self, y: u8, cb: u8, cr: u8) -> (u8, u8, u8) {
        match self {
            ...
            ColourMatrix::Bt2020 => yuv_to_rgb_bt2020(y, cb, cr),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_yuv_to_rgb_black_bt601() {
    ...
    #[test]
    fn test_yuv_to_rgb_white_bt601() {
    ...
    #[test]
    fn test_yuv_to_rgb_black_bt709() {
        let (r, g, b) = yuv_to_rgb_bt709(16, 128, 128);
        assert_eq!((r, g, b), (0, 0, 0));
    }

    #[test]
    fn test_yuv_to_rgb_black_bt2020() {
        let (r, g, b) = yuv_to_rgb_bt2020(16, 128, 128);
        assert_eq!((r, g, b), (0, 0, 0));
    }
    ...
    #[test]
    fn test_colour_matrix_default_is_bt709() {
        // H.265 typically uses BT.709 for HD content
        assert_eq!(ColourMatrix::default(), ColourMatrix::Bt709);
    }
    ...
    fn test_colour_matrix_dispatch() {
        ...
        let bt2020 = ColourMatrix::Bt2020.yuv_to_rgb(16, 128, 128);
        // All should produce black for neutral inputs
        assert_eq!(bt601, (0, 0, 0));
        assert_eq!(bt709, (0, 0, 0));
        assert_eq!(bt2020, (0, 0, 0));
    }
    ...
    fn test_pixel_format_display() {
        assert_eq!(PixelFormat::Yuv420p.to_string(), "YUV 4:2:0 planar (I420)");
        assert_eq!(PixelFormat::Nv12.to_string(), "NV12 semi-planar");
        assert_eq!(PixelFormat::Rgb24.to_string(), "RGB24 packed");
        assert_eq!(PixelFormat::Rgba32.to_string(), "RGBA32 packed");
    }
}
