...
- **Root config files are copied from `modules/devenv/configs/` — never edit the root copies directly.** The devenv `enterShell` hook (in `modules/devenv/configs/default.nix`) populates the project root on every shell entry. All files are copied (via `install -m 644`) as real, writable files rather than symlinked into the read-only Nix store, so tools and AI agents are never prompted for permission to follow symlinks into `/nix/store`. The mapping is:

  | Root file | Source | Method |
  |-|-|-|
  | `biome.jsonc` | `modules/devenv/configs/biome-nix.jsonc` | copy |
  | `deno.jsonc` | `modules/devenv/configs/deno.jsonc` | copy |
  | `lychee.toml` | `modules/devenv/configs/lychee.toml` | copy |
  | `rumdl.toml` | `modules/devenv/configs/rumdl.toml` | copy |
  | `typos.toml` | `modules/devenv/configs/typos.toml` | copy |
  | `.secretsignore` | `modules/devenv/configs/secretsignore` | copy |
  | `.commitlintrc.yml` | `modules/devenv/configs/commitlintrc.nix` | generated (Nix derivation) — scope list is auto-derived from top-level directory names; copied into place |
...
- **To update a config:** edit the source file in `modules/devenv/configs/`, then run `touch .envrc && direnv export json` to regenerate and re-copy the root copies. All root copies are overwritten on each shell entry from their (possibly Nix-store) sources, so changes to generated sources (e.g. `.commitlintrc.yml`) require a devenv re-evaluation to refresh the copied result.
