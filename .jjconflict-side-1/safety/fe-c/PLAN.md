...
- **Checks** (`nix flake check` = CI):
  ...
  - `corpus-rustsec`: pinned vulnerable crate versions, vendored through
    `platform/nix/lib/lib/cargo`'s lockfile handling so the check is pure/offline, asserted
...
| §9: `harden` as a per-crate derivation attribute | Corpus checks drive `RUSTC=` with env vars; `platform/nix/lib/lib/cargo`'s artifact key is untouched (nix-integration §6 Q2 is still the open correctness hazard it was flagged as) |
