...
- Eval unit tests: label/cell resolution, the load-phase target graph, and the
  ...
  ```console
  nix eval -f platform/nix/lib/lib/buck2/tests/labels.nix
  nix eval -f platform/nix/lib/lib/buck2/tests/load.nix
  nix eval -f platform/nix/lib/lib/buck2/tests/analysis.nix
  ```
