...
# against oxidized-file. Compares `oxidized-file`'s output for the sample against
# upstream `file`'s output (both running in the same sandbox, so different
# build hosts don't cause false failures).
#
# Run with: nix build .#checks.x86_64-linux.oxidized-file-test-{type}__{file}
# Example:  nix build .#checks.x86_64-linux.oxidized-file-test-bmp__4x2x24-win3_bmp
{
  pkgs,
  fileTestsSrc,
  type,
...
pkgs.runCommand "oxidized-file-test-${type}-${file}" {
  nativeBuildInputs = [pkgs.oxidized-file-dev pkgs.file pkgs.coreutils pkgs.diffutils pkgs.gnused];
} ''
  ...
  timeout 30 ${pkgs.oxidized-file-dev}/bin/file -b "$SAMPLE" > "$TMPDIR/actual" 2>&1 || true
