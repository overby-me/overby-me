# wiki-dioxus — RadikalWiki (Dioxus / WASM)

A Rust + [Dioxus](https://dioxuslabs.com) 0.7 port of the RadikalWiki frontend
(`web/wiki`), compiled to WebAssembly. It talks to the same NHost / Hasura
backend (auth over REST, data over GraphQL via [cynic](https://cynic-rs.dev)).

See [`PLAN.md`](./PLAN.md) for the roadmap to feature parity with `web/wiki` and
the list of known issues.

## Develop

Everything runs inside the `wiki-dioxus` dev shell (`nix develop .#wiki-dioxus`,
