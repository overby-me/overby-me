...
//!
//! Carried over from the interim backend (`backend/src/symbolicate.rs`) with one
//! change: what it remembers is bounded, since this process does not restart
//! with every deploy the way a serverless container did. `/log` runs every
//! warn and error the app ships through it.

use crate::http::RustlsHttpClient;
use std::collections::VecDeque;
use std::sync::{Arc, Mutex, OnceLock};

/// Builds whose sidecar is kept. Readers are on the newest build or, with a tab
/// left open, the one before; each costs some 25 MB.
const MAX_SIDECARS: usize = 3;

/// Hashes remembered as having NO sidecar. `/log` takes no session, so anyone can
/// name hashes that do not exist: remembering them spares the static host a
/// fetch each, and the cap keeps that memory from being the attack.
const MAX_MISSES: usize = 256;

/// Sidecar BYTES already fetched, by content hash. A build is immutable, so an
/// entry never goes stale; the oldest goes when there are too many.
///
...
/// that actually costs.
///
/// Hits and misses are kept apart so that a flood of made-up hashes cannot push
/// a real build out and turn every real report into a 25 MB download.
#[derive(Default)]
struct Cache {
    sidecars: VecDeque<(String, Arc<Vec<u8>>)>,
    misses: VecDeque<String>,
}

impl Cache {
    /// `Some(None)` is a remembered miss.
    fn get(&self, hash: &str) -> Option<Option<Arc<Vec<u8>>>> {
        if let Some((_, bytes)) = self.sidecars.iter().find(|(h, _)| h == hash) {
            return Some(Some(bytes.clone()));
        }
        self.misses.iter().any(|h| h == hash).then_some(None)
    }

    fn put(&mut self, hash: &str, fetched: Option<Arc<Vec<u8>>>) {
        match fetched {
            Some(bytes) => {
                self.sidecars.push_back((hash.to_string(), bytes));
                if self.sidecars.len() > MAX_SIDECARS {
                    self.sidecars.pop_front();
                }
            }
            None => {
                self.misses.push_back(hash.to_string());
...
pub async fn resolve_stack(client: &RustlsHttpClient, app_origin: &str, stack: &str) -> String {
    ...
    // it the shared sidecar, the rest per-parse state, so a burst of reports
    // doing this at once is how this endpoint could exhaust a small host.
    // Reports are rare enough that serialising them costs nothing worth having.
...
fn rewrite(bytes: &[u8], stack: &str) -> Option<String> {
    ...
    for line in stack.lines() {
        ...
        if resolved.is_empty()
            && let (Some(body), Some(Address::Function(_))) = (offset, frame_address(line))
        {
...
fn first_instruction(bytes: &[u8], body: u64) -> u64 {
    ...
    if pos <= bytes.len() { pos as u64 } else { body }
}
...
async fn sidecar_bytes(
    ...
    if let Some(known) = cache().lock().ok().and_then(|c| c.get(hash)) {
        return known;
    }
    let url = format!("{base_url}/symbols/{hash}.debug.wasm");
    let fetched = async {
        let resp = client.get(&url).ok()?.send().await.ok()?;
        ...
        if !bytes.starts_with(b"\0asm") {
            tracing::warn!(
                "symbols {hash}: not a wasm module ({} bytes), so no sidecar for that build",
                bytes.len()
            );
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn made_up_hashes_cannot_push_a_real_build_out() {
        let mut cache = Cache::default();
        cache.put("real", Some(Arc::new(b"\0asm".to_vec())));
        for n in 0..MAX_MISSES * 2 {
            cache.put(&format!("bogus{n}"), None);
        }
        assert!(
            matches!(cache.get("real"), Some(Some(_))),
            "the build was evicted"
        );
        assert_eq!(cache.misses.len(), MAX_MISSES, "misses grew without bound");
        assert_eq!(cache.get("bogus0"), None, "the oldest miss is forgotten");
        assert_eq!(
            cache.get(&format!("bogus{}", MAX_MISSES * 2 - 1)),
            Some(None)
        );

        for n in 0..MAX_SIDECARS {
            cache.put(&format!("build{n}"), Some(Arc::new(Vec::new())));
        }
        assert_eq!(cache.sidecars.len(), MAX_SIDECARS);
        assert_eq!(
            cache.get("real"),
            None,
            "the oldest build makes room for the newest"
        );
    }
}
