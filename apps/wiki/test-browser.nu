...
def test-shell [session_id: string, timeout: int, passed: int, failed: int]: nothing -> record<passed: int, failed: int> {
    ...
    if not (wd-wait-for-mount $session_id 120) {
...
def test-auth [session_id: string, email: string, password: string, timeout: int, passed: int, failed: int]: nothing -> record<passed: int, failed: int> {
    ...
    # Only a TOP-LEVEL context works here: the check navigates to /(key), which
    # resolves by walking root children, so a nested event's key would 404.
    let nonown_key = (try { wd-execute $session_id ($gql + 'var rt=gql("query{nodes(where:{mimeId:{_eq:\"wiki/home\"}}){id}}",{});var ROOT=null;try{ROOT=rt.data.nodes[0].id}catch(e){}var r=gql("query{members(where:{owner:{_eq:false},accepted:{_eq:true}}){parent{key mimeId parentId}}}",{});var k="none";try{var ms=r.data.members.filter(function(m){return m.parent&&m.parent.parentId==ROOT&&(m.parent.mimeId=="wiki/group"||m.parent.mimeId=="wiki/event")&&/^[a-zA-Z0-9_-]+$/.test(m.parent.key)});if(ms.length)k=ms[0].parent.key}catch(e){}return k;') } catch { "none" })
...
def main [
    ...
    wd-navigate $session_id $"(base-url)/"
    wd-wait-for-mount $session_id 120 | ignore
    wd-execute $session_id 'try{localStorage.clear()}catch(e){}; return "ok"' | ignore
