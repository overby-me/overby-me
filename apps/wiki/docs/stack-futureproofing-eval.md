...
| Realtime / sync transport | SurrealDB LIVE queries | keep-and-extend | The real change-feed is the atproto firehose, not a DB-vendor feature; binding realtime to SurrealDB LIVE re-creates the Hasura lock-in being unwound | Consume Jetstream filtered to `wiki.radikal.*`; push authoritative deltas over one axum WS fed by an in-process broadcast channel (the AppView is a single process holding Turso for both core and view plus the firehose, so no DB LISTEN/NOTIFY is needed) |
...
| Web Server | Caddy → Ferron | change (owner call: Ferron) | Ferron (nixpkgs `ferron`, v2.x KDL) is the chosen reverse-proxy/TLS edge. Honest tradeoff: it is itself a young, small-project Rust web server, so the single-vendor / maturity caution this eval raised for Moella/Kvarn applies to Ferron too — treat it as the edge to MONITOR, not a settled bet | Deploy Ferron as the edge (bundled systemd unit; no upstream NixOS module, so the config is hand-written KDL). Keep terminating-TLS-in-axum as the always-available fallback if Ferron stalls; do not put Ferron on the critical path for a binding election without a soak |
...
| atproto SDK (atrium) | not in README; hand-rolled OAuth | keep-and-extend | Maturest Rust atproto SDK (MIT, moved to the atrium-rs org); replaces the hand-rolled DPoP/PKCE | Adopt atrium-oauth/api/identity behind a thin wrapper; pin exact 0.25.x; validate `wiki.radikal.*` codegen early |
