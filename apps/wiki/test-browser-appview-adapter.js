...
var __s;
try { __s = JSON.parse(localStorage.getItem("wiki_session")); } catch (e) {}
...
function xrpc(method, params, body) {
  ...
  try { return JSON.parse(x.responseText); } catch (e) { return {}; }
}

var KIND = {
  "wiki/folder": "folder", "wiki/document": "document", "wiki/file": "file",
  "vote/policy": "policy", "vote/position": "position", "vote/candidate": "candidate",
  "vote/change": "change", "vote/question": "question", "vote/poll": "poll",
  "wiki/group": "group", "wiki/event": "event"
};
...
function rowOf(n) {
  var data = {};
  if (n.data && typeof n.data === "object") for (var k in n.data) data[k] = n.data[k];
  if (n.content !== undefined && n.content !== null) data.content = n.content;
  return {
    id: n.id, key: n.slug, name: n.title !== undefined ? n.title : n.name,
    contextId: n.context_id || n.id, parentId: n.parent_id || null, kind: n.kind, data: data
  };
}
...
function ballotsOf(poll) {
  ...
  if (!own && p.ballots === 1 && p.counts) {
    own = [];
    p.counts.forEach(function (n, option) { for (var k = 0; k < n; k++) own.push(option); });
  }
  var rows = [];
  for (var i = 0; i < p.ballots; i++) rows.push({ id: poll + "#" + i, data: i === 0 && own ? own : null });
  return rows;
}
...
function seatsNotOwned() {
  ...
  var rows = [];
  ((mine && mine.contexts) || []).forEach(function (c) {
    var seen = xrpc("getNode", { id: c.id });
    if (seen && seen.viewer && !seen.viewer.is_context_owner) {
      rows.push({ parent: { key: c.slug, mimeId: "wiki/" + c.kind, parentId: c.parent_id || null } });
    }
  });
  return rows;
}
...
function gql(q, v) {
  ...
  if (q.indexOf("deleteNode") >= 0) {
    var gone = xrpc("deleteDocument", null, { id: v.i }) || xrpc("deleteContext", null, { id: v.i });
    return { data: { deleteNode: gone ? { id: v.i } : null } };
  }
  // Rules are code here, not rows: there is nothing of the kind to delete.
  if (q.indexOf("deletePermissions") >= 0) return { data: { deletePermissions: { affected_rows: 0 } } };
  if (q.indexOf("deleteMembers") >= 0) return { data: { deleteMembers: { affected_rows: 0 } } };
  if (q.indexOf("insertMembers") >= 0) {
    var put = 0;
    (v.o || []).forEach(function (m) {
      // Whoever makes a context owns it already: the owner row is the interim's.
      if (m.nodeId) { put++; return; }
      var r = xrpc("inviteMembers", null, { context_id: m.parentId, invites: [{ name: m.name, email: m.email }] });
      if (r) put += r.inserted;
    });
    return { data: { insertMembers: { affected_rows: put } } };
  }
  if (q.indexOf("members(where") >= 0) return { data: { members: seatsNotOwned() } };
  ...
  if (q.indexOf("parentId:{_eq:$p}") >= 0) {
    ...
    else if (mime === "vote/comment") rows = ((xrpc("getComments", { on: v.p }) || {}).comments || []).map(function (c) { return { id: c.id }; });
    else if (mime === "speak/list") rows = ((xrpc("listSpeakerLists", { context: v.p }) || {}).lists || []).map(function (l) { return { id: l.id }; });
    else if (mime === "vote/poll") rows = ((xrpc("listPolls", { parent: v.p }) || {}).polls || []).map(function (p) { return { id: p.id }; });
    else rows = childrenOf(v.p).filter(function (n) { return !mime || n.kind === KIND[mime]; });
    if (v.n !== undefined) rows = rows.filter(function (n) { return n.name === v.n; });
  ...
  if (q.indexOf("name:{_eq:$n}") >= 0) {
    ...
    var seen = {};
    var made = ["roots", "mine"].reduce(function (all, scope) {
      return all.concat(((xrpc("listContexts", { scope: scope }) || {}).contexts || []).map(rowOf));
    }, []).filter(function (n) {
      var first = !seen[n.id];
      seen[n.id] = true;
  ...
  return { errors: [{ message: "the AppView adapter has no answer for: " + q }] };
}
